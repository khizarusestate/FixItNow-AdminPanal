...
if (!source.includes(statusConfigAnchor)) {
  throw new Error("STATUS_CONFIG in-progress block not found");
}
const onTheWayConfig = `  "on-the-way": {\n    color: "bg-orange-100 text-orange-700 border-orange-200",\n    icon: MapPin,\n    label: "On The Way",\n  },\n`;
if (!source.includes('"on-the-way": {')) {
  source = source.replace(statusConfigAnchor, `${statusConfigAnchor}\n${onTheWayConfig}`);
}
...
const rejectedCard = `        <StatCard\n          title="Rejected"`;
if (!source.includes(completedCard) || !source.includes(rejectedCard)) {
  throw new Error("Completed/Rejected cards not found");
}
...
if (!source.includes('title="On The Way"')) {
  source = source.replace(completedCard, `${onTheWayCard}${inProgressCard}${completedCard}`);
}

const claimTitle = 'title="Job Claim Request"';
if (!source.includes(claimTitle)) throw new Error("Claim card not found");
source = source.replace(claimTitle, 'title="Booking Claim Request"');
