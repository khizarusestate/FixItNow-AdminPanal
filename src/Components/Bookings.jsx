                    ...
                    {isClaimPending(booking.status) && (
                      <>
                        <button
                          ...
                          title="Approve commission & assign worker"
...
function BookingModal({ booking, processing, onClose }) {
        ...
        <div className="max-h-[70vh] space-y-6 overflow-y-auto p-6">
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              />
              {claimPending && booking.paymentDetails?.commissionAmount > 0 && (
                <InfoCard
                  icon={<Receipt size={16} />}
                  label="Commission due (20%)"
                  value={`₨${Number(booking.paymentDetails.commissionAmount).toLocaleString()}`}
                />
              )}
              {booking.paymentDetails?.commissionTransactionId && (
                <InfoCard
                  ...
                  value={booking.paymentDetails.commissionTransactionId}
                />
              )}
            </div>
          </div>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                ...
                <div className="min-w-0 flex-1 space-y-2">
                  <InfoCard
                    icon={<DollarSign size={16} />}
                    label="Job price"
                    value={`₨${(booking.price || 0).toLocaleString()}`}
                  />
                  <InfoCard
                    icon={<DollarSign size={16} />}
                    label="Commission (20%)"
                    value={`₨${(booking.paymentDetails?.commissionAmount || 0).toLocaleString()}`}
                  />
                </div>
              </div>
            </div>
